...
class Administrator extends CI_Controller {
	...
	function identitaswebsite(){
		...
		if (isset($_POST['submit'])){
            ...
            if ($hasil['file_name']=='' && $hasillogo['file_name']==''){
            	$data = array(
	...
	function kategoris(){
		cek_session_akses('kategoris',$this->session->id_session);
        if ($this->session->level=='admin'){
	...
	// Controller Modul List service

	function listservice(){
		cek_session_akses('listservice',$this->session->id_session);
        if ($this->session->level=='admin'){
	...
	function tambah_listservice(){
		...
		if (isset($_POST['submit'])){
					...
					$config['upload_path'] = 'asset/foto_vendor/';
	        $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG';
	        $config['max_size'] = '1000'; // kb
	        $this->load->library('upload', $config);
	        $this->upload->do_upload('k');
	        $hasil=$this->upload->data();
          $config['source_image'] = 'asset/foto_vendor/'.$hasil['file_name'];
          ...
          $config['source_image'] = 'asset/foto_vendor/'.$img2['file_name'];
          ...
          $config['source_image'] = 'asset/foto_vendor/'.$img3['file_name'];
          ...
          $config['source_image'] = 'asset/foto_vendor/'.$img4['file_name'];
          ...
          $config['source_image'] = 'asset/foto_vendor/'.$img5['file_name'];
            ...
            if ($hasil['file_name']==''){
                    $data = array('id_kategori'=>$this->db->escape_str($this->input->post('a')),
                                    ...
                                    'views'=>'0',
																		...
																		'durasi'=>$this->db->escape_str($this->input->post('durasi')),
                                    'tag'=>$tag,
            ...
            $data['record'] = $this->model_app->view_ordering('kategori','id_kategori','DESC');
			$this->template->load('administrator/template','administrator/mod_service/view_tambah',$data);
		}
	}
	function edit_listservice(){
		cek_session_akses('listservice',$this->session->id_session);
		$id = $this->uri->segment(3);
		if (isset($_POST['submit'])){
					$config['upload_path'] = 'asset/foto_vendor/';
          ...
          $config['source_image'] = 'asset/foto_vendor/'.$img5['file_name'];
			...
			$this->template->load('administrator/template','administrator/mod_service/view_edit',$data);
		}
	}
	function delete_listservice(){
        cek_session_akses('listservice',$this->session->id_session);
        if ($this->session->level=='admin'){
	...
	function listundangan(){
		cek_session_akses('listundangan',$this->session->id_session);
				if ($this->session->level=='admin'){
						$data['record'] = $this->model_app->view_ordering('undangan_tbl','id_berita','DESC');
				}else{
						$data['record'] = $this->model_app->view_where_ordering('undangan_tbl',array('username'=>$this->session->username),'id_berita','DESC');
				}
		$this->template->load('administrator/template','administrator/mod_undangan/views',$data);
	}
	function tambah_listundangan(){
		cek_session_akses('listundangan',$this->session->id_session);
		if (isset($_POST['submit'])){

					$config['upload_path'] = 'asset/foto_undangan/';
	        $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG';
	        $config['max_size'] = '1000'; // kb
	        $this->load->library('upload', $config);
	        $this->upload->do_upload('k');
	        $hasil=$this->upload->data();
          $config['source_image'] = 'asset/foto_undangan/'.$hasil['file_name'];
            ...
            $this->model_app->insert('undangan_tbl',$data);
			redirect('administrator/listundangan');
		}else{
						  $data['get_combo_kategori'] = $this->Kategori_model->get_combo_kategori();
            $data['tag'] = $this->model_app->view_ordering('tag','id_tag','DESC');
            $data['record'] = $this->model_app->view_ordering('kategori_undangan','id_kategori','DESC');
			$this->template->load('administrator/template','administrator/mod_undangan/view_tambah',$data);
		}
	}
	function edit_listundangan(){
		cek_session_akses('listundangan',$this->session->id_session);
		$id = $this->uri->segment(3);
		if (isset($_POST['submit'])){
					$config['upload_path'] = 'asset/foto_undangan/';
			...
			$tag = $this->model_app->view_ordering('tag','id_tag','DESC');
						...
						if ($this->session->level=='admin'){
								 $proses = $this->model_app->edit('undangan_tbl', array('id_berita' => $id))->row_array();
						}else{
								$proses = $this->model_app->edit('undangan_tbl', array('id_berita' => $id, 'username' => $this->session->username))->row_array();
						}
			$data = array('rows' => $proses,'tag' => $tag,'record' => $record);
			$this->template->load('administrator/template','administrator/mod_undangan/view_edit',$data);
		}
	}
	function delete_listundangan(){
        cek_session_akses('listundangan',$this->session->id_session);
        if ($this->session->level=='admin'){
    		$id = array('id_berita' => $this->uri->segment(3));
        }else{
		...
		$this->model_app->delete('undangan_tbl',$id);
		redirect('administrator/listundangan');
	}
	function publish_listundangan(){
        cek_session_admin();
		if ($this->uri->segment(4)=='Y'){
			$data = array('status'=>'N');
		}else{
	...
	function listpromo(){
		cek_session_akses('listpromo',$this->session->id_session);
				...
				$data['rss'] = $this->model_utama->view_join_one('promo_tbl','users','username',0,10);

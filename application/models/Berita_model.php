...
return $this->db->get('Video')->num_rows();
  ...
  function get_one($slug)
    ...
    function update_counter_projek($id)
    {
        //return current article views
        $this->db->where('judul_seo', urldecode($id));
        $this->db->select('views_p'); $count = $this->db->get('projek')->row();
        // then increase by one
        $this->db->where('judul_seo', urldecode($id));
        $this->db->set('views_p', ($count->views_p + 1));
        $this->db->update('projek');
    }
    function update_counter_vendors($id)
    {
        //return current article views
        $this->db->where('namabisnis_seo', urldecode($id));
        $this->db->select('views'); 
        $count = $this->db->get('users_bisnis')->row();
        // then increase by one
        $this->db->where('namabisnis_seo', urldecode($id));
        $this->db->set('views', ($count->views + 1));
        $this->db->update('users_bisnis');
    }
    function update_counter_blogs($id)
    {
        //return current article views
        $this->db->where('judul_seo', urldecode($id));
        $this->db->select('views_s'); 
        $count = $this->db->get('berita')->row();
        // then increase by one
        $this->db->where('judul_seo', urldecode($id));
        $this->db->set('views_s', ($count->views_s + 1));
        $this->db->update('berita');
    }
    function update_counter_harga($id)
    {
        //return current article views
        $this->db->where('judul_seo', urldecode($id));
        $this->db->select('views_h'); $count = $this->db->get('harga')->row();
        // then increase by one
        $this->db->where('judul_seo', urldecode($id));
        $this->db->set('views_h', ($count->views_h + 1));
